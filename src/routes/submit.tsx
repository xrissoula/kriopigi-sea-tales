      ...
      <div className="px-5 max-w-2xl mx-auto">
        {sent ? (
          <div className="rounded-2xl bg-gradient-sea text-primary-foreground p-8 text-center shadow-deep">
            ...
            <p className="mt-2 opacity-90">Your observation has been queued for review. The shore is wider for it.</p>
          </div>
        ) : (
          <form
          ...
          >
            <Field label="Your name (or initials)">
              <input required className="w-full rounded-md border border-input bg-background px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" placeholder="e.g. M. Papadopoulos" />
            </Field>
            <Field label="What did you see?">
              <input required className="w-full rounded-md border border-input bg-background px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" placeholder="Species, behavior, or event" />
            </Field>
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Approx. location">
                <div className="relative">
                  ...
                  <input className="w-full rounded-md border border-input bg-background pl-9 pr-3 py-2.5 text-sm" placeholder="e.g. North Cove" />
                </div>
              </Field>
            </div>
            <Field label="Notes">
              <textarea rows={4} className="w-full rounded-md border border-input bg-background px-3 py-2.5 text-sm resize-none" placeholder="Time of day, weather, behavior, count…" />
            </Field>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="block text-[11px] uppercase tracking-[0.2em] text-muted-foreground mb-1.5">{label}</span>
      {children}
    </label>
  );
}
